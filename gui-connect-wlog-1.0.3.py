...
def carica_configurazioni(filtro_riferimento=None):
    with open("connessioni-vpn.json", "r") as f:
        ...
        if filtro_riferimento:
            configs = [c for c in configs if c.get("riferimento", "") == filtro_riferimento]
        return configs
...
def connetti(cliente, output_callback):
    ...
    tentativi = cliente.get("tentativi_vpn", 5)
    if verifica_connessione(cliente["test_ping"], tentativi=tentativi, output=output_callback):
...
class ConnessioneGUI:
    def __init__(self, root, riferimento=None):
        self.root = root
        self.root.title("Connessione Remota")
        self.root.geometry("500x450")

        self.configs = carica_configurazioni(riferimento)
        ...
        self.memo_box = tk.Text(root, height=5, width=60)
        self.memo_box.pack(pady=5)
        self.memo_box.insert(tk.END, "Memo: seleziona un cliente per vedere le informazioni...")
        self.memo_box.configure(state="disabled")

        self.output = tk.Text(root, height=10, width=60, state="disabled")
        self.output.pack(pady=10)

        self.combo.bind("<<ComboboxSelected>>", self.mostra_memo)
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    app = ConnessioneGUI(root, riferimento)
    root.mainloop()
